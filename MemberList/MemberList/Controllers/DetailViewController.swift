...
final class DetailViewController: UIViewController {
    ...
    // MARK: - 이미지뷰가 눌렀을때의 동작 설정
    
    // 제스처 설정 (이미지뷰가 눌리면, 실행)
    func setupTapGestures() {
        let tapGestures = UITapGestureRecognizer(target: self, action: #selector(touchUpImageView))
        detailView.mainImageView.addGestureRecognizer(tapGestures)
    ...
    @objc func touchUpImageView() {
        ...
        configuration.selectionLimit = 0 // 0은 무한대로 가져온다는 뜻
